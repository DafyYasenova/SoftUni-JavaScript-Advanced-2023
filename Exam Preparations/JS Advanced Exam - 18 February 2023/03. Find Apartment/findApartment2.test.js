...
describe('findNewApartment', () => {
    describe('iisGoodLocation', () => {
        it('if the city and nearPublicTransportation are not a string and boolean', () => {
        ...
        it('Otherwise, if the above conditions are not met', () => {
            expect(findNewApartment.isGoodLocation('Sofia', true)).to.equal("You can go on home tour!");
        });

    });
    describe('isLargeEnough', () => {
        it('If passed apartments parameter is not an array', () => {
            expect(() => { findNewApartment.isLargeEnough(50, 40) }).to.throw("Invalid input!");
        });

        it('If apartments is empty array', () => {
            expect(() => { findNewApartment.isLargeEnough([], 40) }).to.throw("Invalid input!");
        });

        it('If the minimalSquareMeters is not a number', () => {
            expect(() => { findNewApartment.isLargeEnough([40], '40') }).to.throw("Invalid input!");
        });


        it('You must add the area of apartment in resultArr if is equal or bigger than minimalSquareMeters', () => {
    ...
    describe('isItAffordable', () => {
        ...
        it('if price and budget are less or equal to 0', () => {
        ...
        it('You need to calculate if you can afford buying the apartment by subtracting the price of the apartment from your budget', () => {
